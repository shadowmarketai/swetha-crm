...
async def update_tenant(tenant_id: str, body: dict, user: dict = Depends(_require_super_admin)):
    _TENANT_COLS = frozenset([
        "name", "slug", "domain", "plan_id", "is_active", "max_users",
    ...
    updates = {k: v for k, v in body.items() if k in _TENANT_COLS and v is not None}
    if not updates:
        raise HTTPException(400, "No valid fields to update")
    # Safe: column names are validated against frozen allowlist above
...
async def update_user(user_id: str, body: dict, user: dict = Depends(_require_super_admin)):
    ...
    _USER_COLS = frozenset(["name", "role", "is_active", "tenant_id", "plan", "company", "phone", "email"])
    updates = {k: v for k, v in body.items() if k in _USER_COLS and v is not None}
    if not updates:
        raise HTTPException(400, "No valid fields to update")
    # Safe: column names validated against frozen allowlist above
    set_clause = ", ".join(f'"{k}"={_ph}' for k in updates)
...
async def update_platform_ticket(
    ...
    _TICKET_COLS = frozenset(["status", "priority", "assigned_to"])
    updates = {k: v for k, v in body.items() if k in _TICKET_COLS and v is not None}
    if not updates:
        raise HTTPException(400, "No valid fields to update")
    # Safe: column names validated against frozen allowlist above
    set_clause = ", ".join(f'"{k}"={_ph}' for k in updates)
