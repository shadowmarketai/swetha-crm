        ...
        return (
          ...
          >
            <span className="flex items-center gap-2">
              ...
              {tab.label}
              {tab.count != null && (
                <Badge tone={active ? 'brand' : 'default'}>{tab.count}</Badge>
              )}
            </span>
          </button>
        );
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      ...
      {open && (
        <div
          className={cn(
            'absolute z-50 mt-1 min-w-[180px] py-1 rounded-xl bg-white dark:bg-slate-900',
...
export const DropdownItem = ({ icon: Icon, children, danger, onClick, disabled }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    className={cn(
...
export const ConfirmDialog = ({
  ...
  description, confirmText = 'Confirm', cancelText = 'Cancel', variant = 'danger', loading,
}) => (
  <Modal open={open} onClose={onClose} title={title} size="sm"
    footer={
      <>
        <Button variant="secondary" size="sm" onClick={onClose}>{cancelText}</Button>
        <Button variant={variant} size="sm" loading={loading} onClick={onConfirm}>{confirmText}</Button>
      </>
    }
  >
...
/* ──────────────────────────────────────── Pagination ── */
export const Pagination = ({ page, totalPages, total, onPageChange, itemsPerPage }) => {
  if (totalPages <= 1) return null;
  const start = (page - 1) * (itemsPerPage || 10) + 1;
  const end = Math.min(page * (itemsPerPage || 10), total || totalPages * (itemsPerPage || 10));
  return (
    <div className="flex items-center justify-between px-6 py-3 border-t border-slate-100 dark:border-slate-800">
      <span className="text-xs text-slate-500">
        {total ? `${start}–${end} of ${total}` : `Page ${page} of ${totalPages}`}
      </span>
...
export const Spinner = ({ size = 'md', className = '' }) => {
  ...
  return <Loader2 className={cn(sizes[size], 'animate-spin text-slate-400', className)} />;
};
...
export const Toggle = ({ checked, onChange, label, description }) => (
  <div className="flex items-center justify-between py-3">
    ...
    <button
      ...
      style={checked ? { background: 'var(--brand-primary)' } : undefined}
    >
      <span
        className={cn(
