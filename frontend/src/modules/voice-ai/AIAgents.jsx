...
function AgentCardContent({ agent, onConfigure, onToggle, onDelete, canUpdate = true, canDelete = true }) {
  return (
    <>
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 line-clamp-2">{agent.description}</p>

      {/* Stats row */}
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg px-2 py-1.5 text-center">
          <p className="text-sm font-bold text-slate-900 dark:text-white">{agent.totalCalls.toLocaleString()}</p>
      ...
      <div className="flex items-center gap-1 mb-2 flex-wrap">
        ...
        {agent.languages.map((lang) => (
          <span key={lang} className="text-[10px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded">
            {lang}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center gap-1.5 pt-2 border-t border-slate-100 dark:border-slate-700">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg text-xs font-medium hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors"
        >
        ...
        {canUpdate && (
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-lg text-xs font-medium hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors"
          >
        ...
        {canDelete && (
          <button
            ...
            className="px-2 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
          >
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white w-48"
            />
          </div>
          {/* View toggle */}
          <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-0.5">
            <button
              ...
              className={`p-2 rounded-md transition-colors ${viewMode === 'kanban' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
              title="Kanban view"
            >
            ...
            <button
              ...
              className={`p-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
              title="Grid view"
            >
          ...
          {filteredAgents.length === 0 ? (
            ...
            filteredAgents.map((agent) => (
              <div
                ...
                className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 hover:shadow-md hover:border-indigo-300 dark:hover:border-indigo-600 cursor-pointer transition-all"
              >
                <AgentCardContent
                  agent={agent}
                  onConfigure={handleConfigure}
      ...
      {selectedAgent && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          {/* Panel */}
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 h-full overflow-y-auto shadow-2xl animate-slide-in-right">
            {/* Panel header */}
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-slate-600 dark:text-slate-400">{selectedAgent.description}</p>

              {/* Performance */}
              <CollapsibleSection title="Performance" badge={`${Math.round(selectedAgent.successRate * 100)}%`}>
                <div className="grid grid-cols-3 gap-3">
                  <div className="text-center">
                    <div className="flex items-center justify-center w-10 h-10 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg mx-auto mb-1">
                      <Phone className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
                    </div>
                    <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedAgent.totalCalls.toLocaleString()}</p>
                    <p className="text-[10px] text-slate-500">Total Calls</p>
                  </div>
                  <div className="text-center">
                    <div className="flex items-center justify-center w-10 h-10 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg mx-auto mb-1">
                  ...
                  <div className="text-center">
                    <div className="flex items-center justify-center w-10 h-10 bg-amber-50 dark:bg-amber-900/20 rounded-lg mx-auto mb-1">
                      <Clock className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                    </div>
                    <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedAgent.avgDuration}</p>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedAgent.languages.map((lang) => (
                        <span key={lang} className="px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg font-medium">
                          {lang}
                        </span>
                      ))}
                    </div>
              ...
              <CollapsibleSection title="Emotion Handling" badge={`${selectedAgent.emotionRules.length} rules`}>
                <div className="space-y-3">
                  {selectedAgent.emotionRules.map((rule, i) => (
                    <div key={i} className="flex items-start gap-3 p-2.5 bg-slate-50 dark:bg-slate-800 rounded-lg">
                      <EmotionIndicator emotion={rule.emotion} showBar={false} />
                      <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{rule.action}</p>
                    </div>
                  ))}
                </div>
              </CollapsibleSection>
              ...
              <CollapsibleSection title="GenZ Mode" badge={selectedAgent.genZMode ? 'ON' : 'OFF'}>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-700 dark:text-slate-300">GenZ Slang Understanding</span>
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${selectedAgent.genZMode ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'}`}>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {selectedAgent.genZTerms.map((term) => (
                            <span key={term} className="px-2 py-1 bg-pink-50 dark:bg-pink-900/20 text-pink-600 dark:text-pink-400 text-xs rounded-lg font-medium">
                              {term}
                            </span>
                          ))}
                        </div>
              ...
              <div className="flex items-center gap-2 pt-4">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowCreateModal(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Create New Agent</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                />
              </div>
              ...
              {/* Language */}
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Primary Language</label>
                <select
              ...
              {/* Dialect options */}
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Dialect Support</label>
              ...
              {/* Personality */}
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Personality</label>
                <select
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${newAgent.genZMode ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-600'}`}
                >
                  <span
