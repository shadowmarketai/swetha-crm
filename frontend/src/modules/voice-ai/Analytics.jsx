...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg p-3 text-sm">
      ...
      {payload.map((entry, i) => (
        <p key={i} className="text-slate-600 dark:text-slate-400" style={{ color: entry.color }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Voice AI Analytics</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <select
              ...
              className="pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 appearance-none cursor-pointer"
            >
              {DATE_RANGES.map((r) => (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700">
          ...
          <div className="h-64 relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{totalDialectAnalyzed}%</p>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            {DIALECT_DATA.map((item, i) => (
              <div key={i} className="flex items-center justify-between text-sm px-2 py-1.5 bg-slate-50 dark:bg-slate-900 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700">
          ...
          {/* Overall ratio */}
          <div className="text-center mb-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl">
            <p className="text-3xl font-bold text-slate-900 dark:text-white">38.6%</p>
            ...
            {CODE_MIX_DATA.map((item, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-slate-600 dark:text-slate-400">{item.pair}</span>
                </span>
                <span className="font-semibold text-slate-700 dark:text-slate-300">{item.value}%</span>
              </div>
            ))}
              ...
              {SLANG_TABLE_DATA.map((row, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  <td className="px-5 py-3">
                    <span className="font-semibold text-sm text-slate-900 dark:text-white">{row.term}</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className="text-sm text-slate-700 dark:text-slate-300 font-mono">{row.frequency.toLocaleString()}</span>
                  </td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3">
                    <span className="text-sm text-slate-500 dark:text-slate-400 italic truncate block max-w-xs">{row.example}</span>
                  </td>
