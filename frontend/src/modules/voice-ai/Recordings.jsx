...
export default function RecordingsPage() {
  ...
  const RecordingCard = ({ rec }) => {
    ...
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-lg hover:border-indigo-300 dark:hover:border-indigo-600 transition-all group">
        {/* Card header */}
        <div className="px-4 pt-4 pb-3">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm text-slate-900 dark:text-white truncate" title={rec.name}>
                {rec.name}
              </p>
          ...
          <div className="mt-3 space-y-1">
            {rec.transcriptPreview.map((line, i) => (
              <p key={i} className="text-xs text-slate-500 dark:text-slate-400 truncate" title={line}>
                {line}
              </p>
            ))}
          </div>
        ...
        <div className="px-4 py-3 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-700/50">
        ...
        <div className="flex border-t border-slate-100 dark:border-slate-700/50 divide-x divide-slate-100 dark:divide-slate-700/50">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
          >
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-transparent w-56"
            />
          </div>
                    ...
                    <tr key={rec.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-slate-900 dark:text-white truncate max-w-[180px]" title={rec.name}>
                            {rec.name}
                          </span>
                        </div>
                      </td>
                      ...
                      {/* Agent */}
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{rec.agent}</td>

                      {/* Duration */}
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400 font-mono">{rec.duration}</td>
                      ...
                      {/* Date */}
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400 whitespace-nowrap">{rec.date}</td>

                      {/* Size */}
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{rec.size}</td>
      ...
      <div className="flex items-center justify-between px-6 py-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Showing <span className="font-medium text-slate-700 dark:text-slate-300">{filtered.length}</span> recording{filtered.length !== 1 ? 's' : ''}
          {dialectFilter !== 'All' && (
            <span> filtered by <span className="font-medium text-slate-700 dark:text-slate-300">{dialectFilter}</span></span>
          )}
        </p>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Total size: <span className="font-medium text-slate-700 dark:text-slate-300">
