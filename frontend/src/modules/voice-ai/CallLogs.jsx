...
// ── Outcome helpers ──────────────────────────────────────────────────────────
const outcomeConfig = {
  'Meeting Booked':      { icon: CheckCircle2, type: 'success', badge: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' },
  'Query Resolved':      { icon: Info,         type: 'info',    badge: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  'Callback Scheduled':  { icon: Clock,        type: 'warning', badge: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' },
  'Escalated':           { icon: AlertTriangle, type: 'error',  badge: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
  'No Answer':           { icon: XCircle,      type: 'default', badge: 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400' },
};
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="pl-8 pr-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent appearance-none cursor-pointer"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
            >
        ...
        <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-800">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
            />
          </div>
        </div>
                ...
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/60 text-left">
                    {[
                      ...
                      <th
                        key={col.label}
                        className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 whitespace-nowrap ${col.field ? 'cursor-pointer select-none hover:text-indigo-600 dark:hover:text-indigo-400' : ''}`}
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="font-medium text-slate-900 dark:text-white truncate">{call.name}</p>
                              <p className="text-xs text-slate-400">{call.phone}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{call.agent}</td>
                        <td className="px-4 py-3 text-slate-600 dark:text-slate-400 font-mono">{call.duration}</td>
                        <td className="px-4 py-3">
                          <DialectBadge dialect={call.dialect} confidence={call.dialectConfidence} />
                        </td>
                        <td className="px-4 py-3">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Showing <span className="font-medium text-slate-700 dark:text-slate-300">{showingStart}-{showingEnd}</span> of{' '}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium rounded-lg border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
